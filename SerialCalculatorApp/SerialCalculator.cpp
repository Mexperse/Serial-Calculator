...
int main()
{
	std::string PortNo; // Declare a string to hold the port number
	std::cout << "Enter your COM port (e.g., COM3): "; 
	std::getline(std::cin, PortNo); // Read the port number from user input

	// Format for serial port connection
	std::string portFormat = "\\\\.\\" + PortNo; // Join the port number with the prefix
	char* port = const_cast<char*>(portFormat.c_str()); // Convert string to char for SerialPort 
	...
	std::cout << "Connected to Arduino on " << PortNo << std::endl; // Output a success message if the connection was successful
